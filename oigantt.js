...
function updateDate(e) {
    ...
    removeEventSimple(document, 'mouseup', updateDate);
    var evt = e || window.event;

    var newDate = new Date((evt.pageX - document.bar.gantt.div.offsetLeft)* document.bar.gantt.scales[document.bar.gantt.scale] + document.bar.gantt.startDate.getTime());
  //  alert(newDate);
    document.bar.dates[document.dateNumber] = newDate;
    document.bar.draw();

}

GanttBar.prototype.addPhase = function (bardiv, begin, end, className, title, number) {
    ...
    anchor.onmousedown = function () {
        document.bar = this.parentNode.parentNode.bar;
        document.dateNumber = this.parentNode.number;
        addEventSimple(document, 'mouseup', updateDate);
    }
